  ...
  return (
    <div>
      ...
      <div>
        ...
        <input className={"btn btn-success"} value={"Pobierz"} type='button' onClick={handle_button} />
      </div>
    </div>
  );
}
...
function App() {
  ...
  return (
    <div>
      ...
      <div>
        <input className='form-check-input' type='checkbox' defaultChecked={kwiaty} onChange={ () => { setKwiaty(!kwiaty) } } /> Kwiaty
...

      {
        data.map(( element, index ) => {
          ...
          if (!zwierzeta && element.category === 2) { return null; }
          if (!samochody && element.category === 3) { return null; }
          
          const filename = `assets/` + element.filename;
          return (
            <Tile key={element.id} imgAlt={element.alt} imgSrc={filename} downloads={element.downloads} />
          )
        })
      }
    </div>
